...
async function callMcp(
  ...
  // Resolve the MCP hostname via Cloudflare DNS (bypass Tailscale MagicDNS)
  const url = new URL(config.mcpUrl);
  const { Resolver } = await import("node:dns/promises");
  const resolver = new Resolver();
  resolver.setServers(["1.1.1.1", "1.0.0.1"]);
  const [publicIp] = await resolver.resolve4(url.hostname);
  ...
  if (res.status !== 200) {
    throw new Error(`MCP error (${res.status}): ${res.body}`);
  }
